            ...
            if (this.HasPawn)
            {
                Command_Target command_Eject = new()
                {
                    ...
                    action = delegate (LocalTargetInfo target)
                    {
                        ...
                        DMS_AbilityVerb_QuickJump.DoJump(p, destMap, target, new LocalTargetInfo(new IntVec3(p.Position.x, p.Position.y, (p.Position.z+25) < destMap.AllCells.MaxBy(o => o.z).z ? (p.Position.z + 25) : destMap.AllCells.MaxBy(o => o.z).z)), false);
                    }
                };
